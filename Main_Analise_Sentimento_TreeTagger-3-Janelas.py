...
def polaridade_comentarios_palavras(sent_words, sent_words_polarity):
    ...
    file = open(os.path.join("Corpus_Analisados", "TreeTagger_analisados.p"), "rb")
    all_reviews = pickle.load(file)

    print(str(len(all_reviews)))
    mais_comum = []
    subst = []
    aspects = []
    ...
    for i, text in enumerate(all_reviews):
        ...
        for x, token in enumerate(text):
            ...
            if word in sent_words and word not in stop_words:
                ...
                if palavra[x-1] in negacao or palavra[x-2] in negacao:
                    troca = True                        
    ...
    for i, text in enumerate(all_reviews):
        ...
        for x, token in enumerate(text):
            ...
            if word in sent_words and word not in stop_words:
                ...
                if palavra[x-1] in intensificacao or palavra[x-2] in intensificacao:
                    if palavra[x-1] in negacao or palavra[x-2] in  negacao:
                        polaridade = polaridade/3                        
                    else:
                        polaridade = polaridade*3

                elif palavra[x-1] in reducao or palavra[x-2] in reducao:
                    if palavra[x-1] in negacao or palavra[x-2] in  negacao:
                        polaridade = polaridade*3
                    else:
                        polaridade = polaridade/3
                        
                elif palavra[x-1] in negacao or palavra[x-2] in  negacao:
